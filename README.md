...
The `DummyFramework` class is now available directly from `flexible-core`:
...
1. Remove `flexible-dummy-framework` from your `package.json` dependencies
...
The `DummyFramework` class is now located at:
- Source: `flexible-core/src/framework/dummy-framework.ts`
